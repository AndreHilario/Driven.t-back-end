...
async function findByDate(): Promise<[{ date: string }]> {
  const activityDate = (await prisma.$queryRaw(
    Prisma.sql`SELECT DISTINCT DATE("Activity"."startsAt" AT TIME ZONE 'UTC' AT TIME ZONE 'America/Sao_Paulo' ) as dateFROM "Activity"`,
  )) as [{ date: string }];
  return activityDate;
}
...
const activityRepBox = {
  findByDate,
  findByDay,
  findSubscriptionsPerActivity,
  allActivities,
  singleActivityId,
  subscribeOnActivity,
  activitiesSpace,
